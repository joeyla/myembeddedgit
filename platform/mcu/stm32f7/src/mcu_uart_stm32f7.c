...
/*
===============================================================================
Module:        platform.mcu.stm32f7.uart
Description:   STM32F7 UART with RX DMA circular + TX DMA ring + per-write callbacks
Author:        Starter Repo
===============================================================================
*/
/* Includes ------------------------------------------------------------------*/
...
/* Defines -------------------------------------------------------------------*/
#define MODULE_ID   0x303
#define MODULE_TAG  "platform.mcu.stm32f7.uart"

#define RX_DMA_BUF_SIZE 512u
...
struct uart_handle_s {
    ...
    volatile uint8_t* rx_buf; volatile uint16_t rx_tail; volatile uint16_t rx_size; volatile uint16_t rx_mask;
    ...
    volatile uint8_t tx_req_head, tx_req_tail;
...
/* Private (static) data -----------------------------------------------------*/
    ...
    extern volatile uint8_t stm32f7_uart0_rx_buf[];

extern volatile uint8_t stm32f7_uart1_rx_buf[];
extern const uint16_t   stm32f7_uart1_rx_size;
    
    extern volatile uint8_t stm32f7_uart0_tx_buf[];

extern volatile uint8_t stm32f7_uart1_tx_buf[];
extern const uint16_t   stm32f7_uart1_tx_size;
    
    
static struct uart_handle_s handles[1];
...
static void h->tx_coalesce_min = 32;
    ...
    LL_USART_Disable(U);
    uint32_t pclk = (U==USART1 || U==USART6)?LL_RCC_GetUSARTClockFreq(LL_RCC_USART16_CLKSOURCE):LL_RCC_GetUSARTClockFreq(LL_RCC_USART234578_CLKSOURCE);
...
static void dma_rx_start(struct uart_handle_s* h){
    ...
    LL_DMA_ClearFlag_TC1(m->dma); /* stream index may differ, just clear generically */
...
static void tx_dma_start_locked(struct uart_handle_s* h){
    ...
    LL_DMA_DisableStream(h->map->dma, (uint32_t)h->map->dma_tx_stream);
    LL_DMA_SetChannelSelection(h->map->dma_tx_stream, h->map->dma_tx_channel);
    LL_DMA_SetDataTransferDirection(h->map->dma_tx_stream, LL_DMA_DIRECTION_MEMORY_TO_PERIPH);
    LL_DMA_SetStreamPriorityLevel(h->map->dma_tx_stream, LL_DMA_PRIORITY_HIGH);
    LL_DMA_SetMode(h->map->dma_tx_stream, LL_DMA_MODE_NORMAL);
    LL_DMA_SetPeriphIncMode(h->map->dma_tx_stream, LL_DMA_PERIPH_NOINCREMENT);
    LL_DMA_SetMemoryIncMode(h->map->dma_tx_stream, LL_DMA_MEMORY_INCREMENT);
    LL_DMA_SetPeriphSize(h->map->dma_tx_stream, LL_DMA_PDATAALIGN_BYTE);
    LL_DMA_SetMemorySize(h->map->dma_tx_stream, LL_DMA_MDATAALIGN_BYTE);

    LL_DMA_SetPeriphAddress(h->map->dma_tx_stream, LL_USART_DMA_GetRegAddr(h->map->usart, LL_USART_DMA_REG_DATA_TRANSMIT));
    LL_DMA_SetMemoryAddress(h->map->dma_tx_stream, (uint32_t)&h->tx_ring[tail]);
    LL_DMA_SetDataLength(h->map->dma_tx_stream, contiguous);

    LL_DMA_EnableIT_TC(h->map->dma_tx_stream);

    LL_USART_EnableDMAReq_TX(h->map->usart);
    LL_DMA_EnableStream(h->map->dma, (uint32_t)h->map->dma_tx_stream);
    h->tx_busy = 1u;
}
...
status_t uart_open(int logical_id, const uart_config_t *cfg, uart_handle_t** out){
    ...
    struct uart_handle_s* h = &handles[0];
    *h = (struct uart_handle_s){0};
    h->logical_id = logical_id;
    h->map = map;
    /* Bind link-time buffers */
    if(logical_id == 0){
        h->rx_buf = stm32f7_uart0_rx_buf; h->rx_size = stm32f7_uart0_rx_size; h->rx_mask = (uint16_t)(h->rx_size - 1u);
        h->tx_ring = stm32f7_uart0_tx_buf; h->tx_size = stm32f7_uart0_tx_size; h->tx_mask = (uint16_t)(h->tx_size - 1u);
    } else {
        h->rx_buf = stm32f7_uart1_rx_buf; h->rx_size = stm32f7_uart1_rx_size; h->rx_mask = (uint16_t)(h->rx_size - 1u);
        h->tx_ring = stm32f7_uart1_tx_buf; h->tx_size = stm32f7_uart1_tx_size; h->tx_mask = (uint16_t)(h->tx_size - 1u);
    }
...
status_t uart_write(uart_handle_t* h_, const uint8_t *buf, uint16_t len,
    ...
    if(len > space){ os_exit_critical(); return ERR_ERR(MODULE_ID, 0x0011); }
    uint16_t head = h->tx_head;
    ...
    if(rem){ for(uint16_t i=0;i<rem;i++) h->tx_ring[i] = buf[first+i]; }
...
status_t uart_read(uart_handle_t* h_, uint8_t *buf, uint16_t maxlen, uint16_t* out_len){
    ...
    uint16_t tail = h->rx_tail;
    for(uint16_t i=0;i<to_copy;i++){ buf[i] = h->rx_dma_buf[tail]; tail = (tail + 1u) & h->rx_mask; }
    h->rx_tail = tail; *out_len = to_copy; return ERR_OK;
}
status_t uart_set_rx_callback(uart_handle_t* h_, uart_rx_cb_t cb, void *user){
    struct uart_handle_s* h = (struct uart_handle_s*)h_;
    if(!h) return ERR_ERR(MODULE_ID, 0x0013);
    h->rx_cb = cb; h->rx_user = user; return ERR_OK;
}
status_t uart_set_baud(uart_handle_t* h_, uint32_t baud){
    struct uart_handle_s* h = (struct uart_handle_s*)h_; if(!h) return ERR_ERR(MODULE_ID, 0x0014);
    uart_config_t cfg = { .baud=baud, .databits=8, .stopbits=1, .parity=0 }; h->tx_coalesce_min = 32;
    usart_apply_cfg(h->map->usart, &cfg); return ERR_OK;
}

/* IRQ Handlers --------------------------------------------------------------*/
void DMA1_Stream1_IRQHandler(void){ /* RX stream placeholder */
    if(LL_DMA_IsActiveFlag_HT1(DMA1)){ LL_DMA_ClearFlag_HT1(DMA1); }
    if(LL_DMA_IsActiveFlag_TC1(DMA1)){ LL_DMA_ClearFlag_TC1(DMA1); }
}
void DMA1_Stream3_IRQHandler(void){ /* TX stream placeholder */
    if(LL_DMA_IsActiveFlag_TC3(DMA1)){
        LL_DMA_ClearFlag_TC3(DMA1);
        struct uart_handle_s* h = &handles[0];
        os_enter_critical();
        h->tx_tail = (uint16_t)((h->tx_tail + h->tx_active_len) & h->tx_mask);
        uint16_t sent = h->tx_active_len; h->tx_active_len = 0; h->tx_busy = 0;
