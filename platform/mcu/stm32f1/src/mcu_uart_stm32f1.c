...
/*
===============================================================================
Module:        platform.mcu.stm32f1.uart
Description:   STM32F1 UART (USART1) with RX/TX DMA rings + per-write callbacks
Author:        Starter Repo
===============================================================================
*/
/* Includes ------------------------------------------------------------------*/
...
/* Defines -------------------------------------------------------------------*/
#define MODULE_ID   0x302
#define MODULE_TAG  "platform.mcu.stm32f1.uart"

#define RX_DMA_BUF_SIZE 256u  /* F1 smaller buffer example; must be power of two */
...
/* Private (static) data -----------------------------------------------------*/
    ...
    extern volatile uint8_t stm32f1_uart0_rx_buf[];

extern volatile uint8_t stm32f1_uart1_rx_buf[];
extern const uint16_t   stm32f1_uart1_rx_size;
    
    extern volatile uint8_t stm32f1_uart0_tx_buf[];

extern volatile uint8_t stm32f1_uart1_tx_buf[];
extern const uint16_t   stm32f1_uart1_tx_size;
    
    
static struct uart_handle_s handles[1]; /* only USART1 mapped in board */
...
static void h->tx_coalesce_min = 32;
    ...
    LL_USART_Disable(U);
    LL_USART_SetBaudRate(U, SystemCoreClock/ (LL_RCC_GetAPB2Prescaler()==LL_RCC_APB2_DIV_1?1:2), cfg->baud); /* simplistic */
    LL_USART_SetDataWidth(U, LL_USART_DATAWIDTH_8B);
    LL_USART_SetParity(U, (cfg->parity==1)?LL_USART_PARITY_ODD:(cfg->parity==2)?LL_USART_PARITY_EVEN:LL_USART_PARITY_NONE);
    LL_USART_SetStopBitsLength(U, (cfg->stopbits==2)?LL_USART_STOPBITS_2:LL_USART_STOPBITS_1);
    LL_USART_EnableDirectionRx(U);
    LL_USART_EnableDirectionTx(U);
    LL_USART_Enable(U);
}
...
static void dma_rx_start(struct uart_handle_s* h){
    ...
    /* Disable channel */
    LL_DMA_DisableChannel(m->dma, m->dma_rx_channel);
    LL_DMA_ConfigTransfer(m->dma, m->dma_rx_channel,
    ...
    LL_DMA_SetPeriphAddress(m->dma, m->dma_rx_channel, LL_USART_DMA_GetRegAddr(m->usart, LL_USART_DMA_REG_DATA_RECEIVE));
    LL_DMA_SetMemoryAddress(m->dma, m->dma_rx_channel, (uint32_t)h->rx_buf);
    LL_DMA_SetDataLength(m->dma, m->dma_rx_channel, h->rx_size);
    LL_DMA_EnableIT_HT(m->dma, m->dma_rx_channel);
    LL_DMA_EnableIT_TC(m->dma, m->dma_rx_channel);
    LL_USART_EnableDMAReq_RX(m->usart);
    LL_DMA_EnableChannel(m->dma, m->dma_rx_channel);
}

static uint16_t dma_rx_available(struct uart_handle_s* h){
    uint16_t ndtr = (uint16_t)LL_DMA_GetDataLength(h->map->dma, h->map->dma_rx_channel);
...
static void tx_dma_start_locked(struct uart_handle_s* h){
    ...
    LL_DMA_DisableChannel(h->map->dma, h->map->dma_tx_channel);
    LL_DMA_ConfigTransfer(h->map->dma, h->map->dma_tx_channel,
        ...
        LL_DMA_MDATAALIGN_BYTE |
        LL_DMA_PRIORITY_HIGH);
    LL_DMA_SetPeriphAddress(h->map->dma, h->map->dma_tx_channel, LL_USART_DMA_GetRegAddr(h->map->usart, LL_USART_DMA_REG_DATA_TRANSMIT));
    LL_DMA_SetMemoryAddress(h->map->dma, h->map->dma_tx_channel, (uint32_t)&h->tx_ring[tail]);
    LL_DMA_SetDataLength(h->map->dma, h->map->dma_tx_channel, contiguous);
    LL_DMA_EnableIT_TC(h->map->dma, h->map->dma_tx_channel);

    LL_USART_EnableDMAReq_TX(h->map->usart);
    LL_DMA_EnableChannel(h->map->dma, h->map->dma_tx_channel);
    h->tx_busy = 1u;
}
...
status_t uart_open(int logical_id, const uart_config_t *cfg, uart_handle_t** out){
    ...
    *h = (struct uart_handle_s){0};
    ...
    if(logical_id == 0){
        h->rx_buf = stm32f1_uart0_rx_buf; h->rx_size = stm32f1_uart0_rx_size; h->rx_mask = (uint16_t)(h->rx_size - 1u);
        h->tx_ring = stm32f1_uart0_tx_buf; h->tx_size = stm32f1_uart0_tx_size; h->tx_mask = (uint16_t)(h->tx_size - 1u);
    } else {
        h->rx_buf = stm32f1_uart1_rx_buf; h->rx_size = stm32f1_uart1_rx_size; h->rx_mask = (uint16_t)(h->rx_size - 1u);
        h->tx_ring = stm32f1_uart1_tx_buf; h->tx_size = stm32f1_uart1_tx_size; h->tx_mask = (uint16_t)(h->tx_size - 1u);
    }
...
status_t uart_close(uart_handle_t* h_){
    ...
    LL_DMA_DisableChannel(h->map->dma, h->map->dma_rx_channel);
...
status_t uart_write(uart_handle_t* h_, const uint8_t *buf, uint16_t len,
    ...
    if(len > space){
        os_exit_critical();
        return ERR_ERR(MODULE_ID, 0x0011);
    }
    uint16_t head = h->tx_head;
...
/* IRQ Handlers --------------------------------------------------------------*/
void DMA1_Channel5_IRQHandler(void){ /* RX complete/half */
